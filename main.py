...
def send_welcome(message):
    bot.reply_to(message,
                 "Hello! I'm Bot That will help you to convert currencies!\n If you want to start,  type /exchange")
...
def currency_list(message):
    bot.reply_to(message, f"Here's currency codes supported by us:\n {currency}")
...
def currency_out_handler(message):
    ...
    try:
        if not currency_in.upper() in currency:
            raise Exception("Error")
        logging.info(f"user chose {currency_in} currency")

    except:
        logging.error(f"ERROR: Wrong currency selected: {currency_in}")
        sent_msg = bot.send_message(
            message.chat.id, f"Wrong currency selected: {currency_in} \n If you want to start again, type /start", parse_mode="Markdown")
...
def amount_handler(message, currency_in):
    ...
    try:
        if not currency_out.upper() in currency:
            raise Exception("Error")
        logging.info(f"user chose {currency_out} currency")

    except:
        logging.error(f"ERROR: Wrong currency selected: {currency_out}")
        sent_msg = bot.send_message(
            message.chat.id, f"Wrong currency selected: {currency_out} \n If you want to start again, type /start", parse_mode="Markdown")
        bot.register_next_step_handler(
            sent_msg)
    else:
...
def currency_fetch(message, currency_in, currency_out):
    try:
        ...
        logging.error(f"ERROR: amount should be a number, not: {message.text}")
        sent_msg = bot.send_message(
            message.chat.id, f"ERROR: amount should be a number, not: {message.text} \n If you want to start again, type /start", parse_mode="Markdown")
        bot.register_next_step_handler(
            sent_msg)
    else:
